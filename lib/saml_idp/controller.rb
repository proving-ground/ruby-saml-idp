  ...
  module Controller
    ...
    protected
      ...
      def encode_response(request_id, request_acs_url, request_issuer_name, opts = {})
        ...
        assertion = %[<saml:Assertion xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" ID="_#{assertion_id}" Version="2.0" IssueInstant="#{now.iso8601}"><saml:Issuer>#{idp_uri}</saml:Issuer><saml:Subject><saml:NameID SPNameQualifier="#{request_issuer_name}" Format="urn:oasis:names:tc:SAML:2.0:nameid-format:transient">_#{request_issuer_id}</saml:NameID><saml:SubjectConfirmation Method="urn:oasis:names:tc:SAML:2.0:cm:bearer"><saml:SubjectConfirmationData NotOnOrAfter="#{after_now.iso8601}" Recipient="#{request_acs_url}" InResponseTo="#{request_id}"/></saml:SubjectConfirmation></saml:Subject><saml:Conditions NotBefore="#{before_now.iso8601}" NotOnOrAfter="#{after_now.iso8601}"><saml:AudienceRestriction><saml:Audience>#{request_issuer_name}</saml:Audience></saml:AudienceRestriction></saml:Conditions><saml:AuthnStatement AuthnInstant="#{now.iso8601}" SessionNotOnOrAfter="#{timeout.iso8601}" SessionIndex="_#{session_id}"><saml:AuthnContext><saml:AuthnContextClassRef>urn:oasis:names:tc:SAML:2.0:ac:classes:Password</saml:AuthnContextClassRef></saml:AuthnContext></saml:AuthnStatement><saml:AttributeStatement><saml:Attribute Name="firstName" NameFormat="urn:oasis:names:tc:SAML:2.0:attrname-format:uri"><saml:AttributeValue xsi:type="xs:string">#{opts[:first_name]}</saml:AttributeValue></saml:Attribute><saml:Attribute Name="lastName" NameFormat="urn:oasis:names:tc:SAML:2.0:attrname-format:uri"><saml:AttributeValue xsi:type="xs:string">#{opts[:last_name]}</saml:AttributeValue></saml:Attribute><saml:Attribute Name="email" NameFormat="urn:oasis:names:tc:SAML:2.0:attrname-format:uri"><saml:AttributeValue xsi:type="xs:string">#{opts[:email]}</saml:AttributeValue></saml:Attribute><saml:Attribute Name="externalId" NameFormat="urn:oasis:names:tc:SAML:2.0:attrname-format:uri"><saml:AttributeValue xsi:type="xs:string">#{opts[:account_id]}</saml:AttributeValue></saml:Attribute></saml:AttributeStatement></saml:Assertion>]
